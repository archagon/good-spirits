    ...
    public func populateWithSampleData()
    {
            ...
            let data = try? Data.init(contentsOf: URL.init(fileURLWithPath: path))
        {
            ...
            for datum in sampleData
            {
                ...
                let drink = Model.Drink.init(name: datum.drink_name, style: DrinkStyle.init(rawValue: datum.drink_type)!, abv: datum.drink_abv, price: datum.drink_price, volume: Measurement.init(value: datum.drink_volume, unit: UnitVolume.init(symbol: datum.drink_volume_unit)))
